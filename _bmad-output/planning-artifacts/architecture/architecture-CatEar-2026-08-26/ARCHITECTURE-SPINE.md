...
- **Binds:** Medidor de Habilidade, Medidor de Esforço, Skill Tree (estado dos nós, incluindo o nó capstone de Audiação), Baseline Dia 1, dificuldade adaptativa, histórico de variações recentes por tipo de exercício (FR-6), nível de partida do nivelamento (Story 1.9, tabela `placements`), tabela `telemetry_events` (dono: Telemetria — ver AD-6).
...
- **Rule:** o módulo **Progressão** é o único autorizado a escrever Medidor de Habilidade, Medidor de Esforço, estado de nós do Skill Tree (enum: `bloqueado | disponível | completo | em_reforço | capstone_futuro` — o nó de Audiação nasce fixo em `capstone_futuro`, nunca navegável na v1), Baseline, decisão de dificuldade adaptativa, o histórico de variações recentes por tipo de exercício e o **nível de partida** (Story 1.9: o Nivelamento produz o nível e o grava só pelo port `PlacementRepository` do `domain/` da Progressão; o gate de boot o lê por `placementProvider`; a Story 2.1 abre o skill tree a partir dele). Qualquer outro módulo só lê esses dados via `domain/` do Progressão.
