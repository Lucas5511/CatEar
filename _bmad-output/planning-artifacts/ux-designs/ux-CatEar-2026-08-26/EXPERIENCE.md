...
| Offline / sem conexão | Sessão de Exercício | Áudio pré-renderizado já embarcado no app permite continuar a sessão normalmente; o progresso é local (persistido via Drift), sem sync entre dispositivos na v1 (sync é v2). [ASSUMPTION] as amostras de áudio são embarcadas no bundle do app, sem download remoto na v1 (decisão de design técnico — Story 1.3/1.3b). |
