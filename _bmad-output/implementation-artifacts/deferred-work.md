...
- source_spec: `_bmad-output/implementation-artifacts/spec-1-5a-seam-type-agnostic-do-fluxo-de-pratica.md`
  ...
  **Decisões já tomadas pelo humano em 2026-09-05, para a spec da 1.5 herdar sem reabrir:**
  1. **Forma do áudio (AC2 "contexto musical"):** uma forma por tipo, derivada dos refs. Acorde: bloco → arpejo → bloco — a 1.4b guardou `audioSampleRefs` como `[bloco, fundamental, terça, quinta]` exatamente para isso. Escala: as 8 notas em sequência (~3,6 s no `noteGap` atual de 450 ms; avaliar gap por tipo). Rejeitadas: motif de 3 eventos para todos (uma escala reduzida a 3 notas não é reconhecível como modo) e bloco isolado (é o que a AC2 proíbe).
  2. **Loop:** único, de 23 para 39 exercícios (23 intervalos + 8 acordes + 8 escalas), filtrando por `requiresVoice == false` — o que deixa os 2 exercícios `resolution` de fora, por serem do Epic 3. A sessão fica ~70% mais longa até a Story 1.7 chegar; truncar aqui seria antecipar a decisão que é o objeto da 1.7.

## Deferred from: step-04 review de spec-1-5a (2026-09-05, review_loop 0)
